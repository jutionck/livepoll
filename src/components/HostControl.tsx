...
import {
  Play, RefreshCw, Users, AlertCircle,
  Copy, Check, ChevronRight, LogOut, Eye, Lock
} from 'lucide-react';
...
export const HostControl: React.FC<HostControlProps> = ({ code, navigate, theme, toggleTheme }) => {
  ...
  useEffect(() => {
    if (!hostToken) { setError('Token Host tidak ditemukan.'); return; }
    fetchSession();
  }, [code, hostToken]);
  ...
  const fetchSession = async () => {
    try {
      ...
      else if (data.questions && Object.keys(data.questions).length > 0) setSelectedQuestionId(Object.keys(data.questions)[0]);
  ...
  const fetchResults = async (qId: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/results?code=${code}&q=${qId}&t=${Date.now()}`, {
        signal: abortControllerRef.current.signal, headers: { 'Cache-Control': 'no-store' },
      });
      const data = await res.json();
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-sm w-full bg-white border border-slate-200 rounded-xl p-6 text-center shadow-sm">
          <div className="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center mx-auto mb-4"><AlertCircle className="text-red-500" size={24} /></div>
          <h2 className="text-sm font-bold text-slate-900 mb-1">Akses Ditolak</h2>
          <p className="text-xs text-slate-500 mb-5">{error}</p>
          <button onClick={() => navigate('#/')} className="w-full bg-slate-900 text-white font-semibold text-xs py-2 rounded-lg">Kembali</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-dots flex flex-col font-sans">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 py-3 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-20 gap-2">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xs font-bold text-slate-900 dark:text-white leading-none truncate max-w-[120px] sm:max-w-none">{session.title}</h1>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className={`w-1.5 h-1.5 rounded-full ${session.status === 'active' ? 'bg-green-500 shrink-0' : 'bg-slate-350 dark:bg-slate-700'} shrink-0`}></span>
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-12 gap-5 animate-fade-in">
        ...
        <div className="lg:col-span-4 space-y-5 order-2 lg:order-1">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col items-center text-center shadow-sm">
            <h3 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4">Informasi Sesi</h3>
            ...
            <div className="w-full">
              <div className="bg-slate-50 dark:bg-slate-950 px-2.5 py-1.5 border border-slate-100 dark:border-slate-800 rounded-lg flex items-center justify-between gap-2 mb-2">
                ...
                <button onClick={copyUrl} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-250 p-1 shrink-0">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
            <h3 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Daftar Pertanyaan</h3>
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className={`text-[9px] font-bold uppercase ${isSelected ? 'text-slate-400 dark:text-slate-500' : 'text-slate-350 dark:text-slate-600'}`}>Q{idx + 1}</span>
                        {isActive && (
                          <span className={`text-[8px] font-bold px-1 py-0.2 rounded uppercase ${isSelected ? 'bg-white/10 dark:bg-slate-800 text-white dark:text-slate-200 border border-white/20 dark:border-slate-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-750'}`}>
                            AKTIF
                          </span>
                        )}
                      </div>
                    ...
                    <ChevronRight size={12} className={`${isSelected ? 'text-slate-400 dark:text-slate-500' : 'text-slate-300 dark:text-slate-600'} shrink-0`} />
                  </button>
                );
              })}
            </div>
        ...
        <div className="lg:col-span-8 space-y-5 order-1 lg:order-2">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
            <div className="mb-4 flex items-start justify-between gap-4">
              ...
              {selectedQuestionId === session.active_question_id && timeLeft !== null && (
                <div className={`px-2.5 py-1 rounded border text-xs font-bold shrink-0 ${timeLeft > 0 ? 'bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900/50 animate-pulse' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-450 border border-slate-200 dark:border-slate-705'}`}>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider">Hasil Polling</h3>
              ...
              {activeQuestion?.type === 'rating' ? (
                <div className="flex flex-col md:flex-row gap-8 items-center">
                  <div className="text-center md:border-r border-slate-100 dark:border-slate-800 md:pr-8 py-3 shrink-0">
                    <p className="text-4xl font-extrabold text-slate-900 dark:text-white">{resultsData?.average_rating || 0}</p>
                    <div className="flex justify-center gap-0.5 my-1.5 text-amber-400">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <span key={s} className={`text-xl ${s <= Math.round(resultsData?.average_rating || 0) ? 'text-amber-400' : 'text-slate-200 dark:text-slate-800'}`}>★</span>
                      ))}
                    </div>
                    <p className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Rata-rata</p>
                  </div>
                    ...
                    {[5, 4, 3, 2, 1].map((r) => {
                      ...
                      return (
                        <div key={r} className="flex items-center gap-3">
                          <span className="w-8 text-[11px] font-bold text-slate-400 dark:text-slate-550 text-right">{r} ★</span>
                          <div className="flex-1 h-2.5 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden">
                            <div className="h-full bg-slate-450 dark:bg-slate-500 rounded-full" style={{ width: `${pct}%` }}></div>
                          </div>
                          <span className="w-8 text-right text-xs font-bold text-slate-500 dark:text-slate-400">{count}</span>
                        </div>
                      );
                    })}
                  </div>
                      ...
                      <div key={key}>
                        <div className="flex justify-between text-xs font-bold text-slate-750 dark:text-slate-300 mb-1.5">
                          <span className="flex items-center gap-2 font-medium text-slate-800 dark:text-slate-200">
                            <span className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 text-slate-650 dark:text-slate-400 text-[9px] font-black flex items-center justify-center uppercase border border-slate-200 dark:border-slate-700">{key}</span>
                            {label}
                          </span>
                          <span className="text-slate-500 dark:text-slate-450 font-bold">{count} ({Math.round(pct)}%)</span>
                        </div>
                        <div className="w-full h-3 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden">
                          <div className="h-full bg-slate-900 dark:bg-white rounded-full" style={{ width: `${pct}%` }}></div>
                        </div>
                      </div>
                    );
                  })}
      ...
      {notification && (
        <div className="fixed top-16 right-6 z-35 animate-fade-in">
          <div className={`px-4 py-2.5 rounded-lg border text-xs font-semibold shadow-md flex items-center gap-2 ${
      ...
      {showExitModal && (
        <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-40 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 max-w-xs w-full shadow-lg text-slate-900 dark:text-white">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">Sesi polling akan tetap berjalan di server. Anda dapat kembali menggunakan kode sesi dan token host Anda.</p>
      ...
      {showResetModal && (
        <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-40 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 max-w-xs w-full shadow-lg text-slate-900 dark:text-white">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">Hapus semua jawaban masuk untuk pertanyaan ini? Tindakan ini tidak dapat dibatalkan.</p>
            <div className="flex gap-2 justify-end">
              <button
