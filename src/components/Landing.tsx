...
import {
  PlusCircle, ArrowRight, QrCode, Shield, Layers,
  Tv, Users, Clock, Star, CheckCircle, ChevronRight, Copy, Heart
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-dots flex flex-col font-sans selection:bg-slate-200 dark:selection:bg-slate-800">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base leading-relaxed max-w-lg">
              Buat presentasi interaktif untuk webinar, seminar, atau kelas dalam hitungan detik. Audiens berpartisipasi langsung dari browser HP mereka tanpa perlu mengunduh aplikasi.
            </p>
          ...
          <div className="lg:col-span-5">
            <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-xs text-slate-500 dark:text-slate-450 uppercase tracking-wider">Untuk Peserta</h3>
              </div>
      ...
      <section className="py-20 px-6 max-w-5xl mx-auto w-full">
        <div className="text-center mb-16">
          ...
          <p className="text-xs text-slate-450 dark:text-slate-400">Tiga langkah sederhana untuk memulai polling interaktif</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-4">
            {[
              ...
              >
                <h3 className={`font-bold text-sm ${activeStep === item.step ? 'text-slate-950 dark:text-white' : 'text-slate-700 dark:text-slate-400'}`}>
                  {item.title}
                </h3>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm min-h-[380px] flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 h-8 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center px-4 gap-1.5 shrink-0">
              ...
              <div className="w-2.5 h-2.5 rounded-full bg-slate-350 dark:bg-slate-650"></div>
              <div className="w-2.5 h-2.5 rounded-full bg-slate-350 dark:bg-slate-650"></div>
              <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 ml-2">Preview Screen - Langkah {activeStep}</span>
            </div>
              ...
              {activeStep === 1 && (
                <div className="space-y-4 animate-fade-in text-left">
                  <div className="border border-slate-200 rounded-lg p-4 bg-white space-y-3 shadow-xs">
                    <div>
                      <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Pertanyaan</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tipe</label>
                        <div className="px-3 py-1.5 border border-slate-200 rounded-lg text-[10px] font-bold bg-white">
                          Pilihan Tunggal
                        </div>
                      </div>
                      <div>
                        <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Timer</label>
                        <div className="px-3 py-1.5 border border-slate-200 rounded-lg text-[10px] font-bold bg-white">
                          30 Detik
                        </div>
                      </div>
                    ...
                    <div className="space-y-1.5 pt-1.5">
                      <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Opsi Pilihan</label>
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded bg-slate-100 text-slate-400 text-[9px] font-black flex items-center justify-center border border-slate-200">A</span>
                        <div className="flex-1 px-3 py-1 border border-slate-200 rounded-lg text-[10px] bg-white font-medium">Sangat Efisien</div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded bg-slate-100 text-slate-400 text-[9px] font-black flex items-center justify-center border border-slate-200">B</span>
                        <div className="flex-1 px-3 py-1 border border-slate-200 rounded-lg text-[10px] bg-white font-medium">Cukup Efisien</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {activeStep === 2 && (
                <div className="space-y-4 animate-fade-in text-left max-w-sm mx-auto w-full">
                  <div className="bg-white border border-slate-200 p-5 rounded-xl text-center flex flex-col items-center shadow-xs">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">Informasi Sesi Bergabung</h4>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mb-4">
                      <svg width="110" height="110" className="text-slate-900" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <div className="w-full">
                      <div className="bg-slate-50 px-2.5 py-1.5 border border-slate-100 rounded-lg flex items-center justify-between gap-2 mb-3">
                        <span className="text-slate-500 text-[10px] font-mono truncate">livepoll.com/#/join/MG84BX</span>
                        <div className="text-slate-400 p-1 shrink-0"><Copy size={12} /></div>
                      </div>
              ...
              {activeStep === 3 && (
                <div className="space-y-4 animate-fade-in text-left bg-white dark:bg-slate-850 p-5 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs text-slate-900 dark:text-white">
                  <div className="flex justify-between items-center pb-2.5 border-b border-slate-100 dark:border-slate-750 mb-3 text-slate-400 dark:text-slate-500 text-[9px] font-bold">
                    ...
                    <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> 10 Respon</span>
                  </div>
                  <h3 className="text-xs font-bold text-slate-900 dark:text-white mb-4">Bagaimana efisiensi kode dengan AI?</h3>
                  <div className="space-y-3.5 flex-grow text-slate-700 dark:text-slate-300">
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] font-bold text-slate-700 dark:text-slate-300">
                        <span className="flex items-center gap-1.5"><span className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 text-slate-650 dark:text-slate-350 text-[9px] font-black flex items-center justify-center uppercase border border-slate-200 dark:border-slate-750">A</span> Sangat Efisien</span>
                        <span className="text-slate-500 dark:text-slate-400">7 <span className="text-slate-400 dark:text-slate-500 text-[9px] font-normal">(70%)</span></span>
                      </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] font-bold text-slate-700 dark:text-slate-300">
                        <span className="flex items-center gap-1.5"><span className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 text-slate-650 dark:text-slate-350 text-[9px] font-black flex items-center justify-center uppercase border border-slate-200 dark:border-slate-750">B</span> Cukup Efisien</span>
                        <span className="text-slate-500 dark:text-slate-400">2 <span className="text-slate-400 dark:text-slate-500 text-[9px] font-normal">(20%)</span></span>
                      </div>
                      <div className="w-full h-2.5 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden">
                        <div className="h-full bg-slate-400 dark:bg-slate-300 rounded-full" style={{ width: '20%' }}></div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* Features Section */}
      <section id="features" className="bg-white dark:bg-slate-900/60 border-t border-b border-slate-200 dark:border-slate-800 py-16 px-6">
        <div className="max-w-5xl mx-auto w-full">
          <div className="text-center mb-12">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-2">Mengapa Memilih LivePoll?</h2>
            <p className="text-xs text-slate-450 dark:text-slate-400">Kemudahan maksimal untuk presenter dan peserta</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Clock, title: 'Real-time 1 Detik', desc: 'Hasil respon ter-update otomatis pada layar presentasi setiap 1 detik tanpa refresh halaman manual.' },
              { icon: QrCode, title: 'Akses Instan QR', desc: 'Audiens cukup scan QR code atau masukkan kode sesi dari browser ponsel untuk bergabung secara instan.' },
              { icon: Shield, title: 'Keamanan Data', desc: 'Vote diverifikasi menggunakan participant ID unik dan server-side validation untuk mencegah duplikasi.' },
              { icon: Tv, title: 'Mode Presentasi Layar Penuh', desc: 'Panel presentasi dirancang khusus untuk proyektor dan screenshare dengan latar kontras tinggi.' },
              { icon: Star, title: 'Tipe Polling Beragam', desc: 'Mendukung pilihan tunggal, pilihan ganda, dan rating bintang 1-5 yang fleksibel.' },
              { icon: CheckCircle, title: 'Tanpa Registrasi', desc: 'Peserta tidak perlu membuat akun atau memasukkan email. Cukup scan dan vote.' }
            ].map((feat, idx) => (
              <div key={idx} className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex items-start gap-4 hover:border-slate-350 dark:hover:border-slate-700 transition-colors bg-white dark:bg-slate-900">
      ...
      <section className="py-16 px-6 max-w-3xl mx-auto w-full text-center space-y-6">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Siap Untuk Meningkatkan Keterlibatan Audiens?</h2>
        <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm max-w-md mx-auto leading-relaxed">
          Gunakan LivePoll sekarang secara gratis untuk webinar, seminar, presentasi kelas, atau rapat tim Anda berikutnya.
        </p>
        <div>
          <button
            onClick={() => navigate('#/host/new')}
